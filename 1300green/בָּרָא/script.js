...
function startScale(e, id, corner) {
  ...
  selectLayer(id);
  const layer = getLayer(id);
  activeAction = { type: 'scale', id, corner,
    startMouseX: e.clientX, startMouseY: e.clientY,
...
function startRotate(e, id) {
  ...
  activeAction = { type: 'rotate', id, cx, cy,
...
async function doTake() {
  ...
  for (const layer of sorted) {
    ...
    const cx = layer.x + (iw * layer.scale) / 2;
    const cy = layer.y + (ih * layer.scale) / 2;
    ctx.save();
    ctx.translate(cx, cy);
